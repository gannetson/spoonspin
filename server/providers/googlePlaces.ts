...
import { searchGoogleRestaurantsByCuisine } from "../lib/googlePlacesLookup.ts";
import type { LiveRestaurantProvider, ProviderSearchInput } from "./types.ts";
import { locationHint } from "./types.ts";

export function createGooglePlacesProvider(apiKey: string): LiveRestaurantProvider {
  ...
  return {
    ...
    async search(input: ProviderSearchInput): Promise<Restaurant[]> {
      ...
      const cities = location.toLowerCase() === "netherlands" ? undefined : [location];
