...
describe("namesLikelyMatch", () => {
  it("matches substring and token overlap", () => {
    expect(namesLikelyMatch("Restaurant Blauw", "Restaurant Blauw Amsterdam")).toBe(true);
    expect(namesLikelyMatch("Kantjil & de Tijger", "Kantjil en de Tijger")).toBe(true);
...
describe("isInNetherlands", () => {
  it("accepts country labels", () => {
    expect(isInNetherlands("Amstelveenseweg 158, 1075 XN Amsterdam, Netherlands")).toBe(
      true,
    );
  ...
  it("accepts Dutch postcodes when country is omitted", () => {
    ...
    expect(isInNetherlands("Amstelveenseweg 158-160, 1075 XN Amsterdam")).toBe(true);
...
describe("extractCity", () => {
  it("reads city when country is present", () => {
    expect(extractCity("Amstelveenseweg 158, 1075 XN Amsterdam, Netherlands")).toBe(
      "Amsterdam",
    );
  });

  it("reads city when country is omitted", () => {
    expect(extractCity("Amstelveenseweg 158-160, 1075 XN Amsterdam")).toBe("Amsterdam");
    expect(extractCity("Spuistraat 291 A, 1012 VS Amsterdam")).toBe("Amsterdam");
  });
});
